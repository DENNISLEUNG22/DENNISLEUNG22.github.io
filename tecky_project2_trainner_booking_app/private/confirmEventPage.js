...
window.onload = async () => {
        ...
        <p>
            <button class="btn btn-primary" id="joinEvent-${v.id}">立即參加</button>
        </p>
         </div>
         </div>
         </div>
    ...
    const joinEventBtn = document.getElementById("joinEvent-" + eventID);
    console.log(eventID);

    joinEventBtn.addEventListener('click',async()=>{
        console.log(eventID);
        const response = await fetch("/joinEvent/" + eventID, {
            method:"post",

            //no need header and body because there is no input from frontend
            // headers:{
